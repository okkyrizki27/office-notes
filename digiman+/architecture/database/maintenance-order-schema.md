...
```
> **⚠️ Temuan penting (15 Jul 2026)**: `HourMeter`, `InspectorCode`, `InspectorName` **sudah ada** di tabel ini — field ini **sudah masuk payload SAP hari ini**, bukan gap baru. Yang belum jelas: apakah nilainya di-populate dari cross-service call live (mis. `GetWorkOrderById` atau call sejenis ke `TaskPersonalized`) saat `PoolingMOItem` dibangun (5.2), atau ada mekanisme lain. Ini mengubah framing [maintenance-activity-type-enhancement.md](../inspection-order/maintenance-activity-type-enhancement.md) 2.9 — bukan "menambah field baru ke payload", tapi "mengubah sumber field yang sudah ada dari live-resolve jadi snapshot-at-creation".
