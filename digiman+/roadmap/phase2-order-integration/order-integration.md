...
**⚠️ Accepted Risk (2026-08-19) — window pending approval tidak terlihat correlation.** Antara submit dan final approval, eMOL belum punya row di `PoolingMOItem` maupun `MOOpen`, jadi **tidak muncul sebagai kandidat**. Ini sisi lain dari "tersaring dengan sendirinya" di atas: penyaringannya benar, tapi berarti ada window buta. Kalau ada Finding lain untuk defect yang sama di komponen yang sama dalam window itu, correlation mengembalikan kosong dan terbentuk **Order kedua**.

**Dua batasan operasional mempersempitnya (konfirmasi user, 2026-08-19):**

1. **Hanya 1 Form aktif per shift per unit** — jadi tidak ada dua Form paralel di unit yang sama yang bisa saling tidak terlihat. Di dalam satu Form, [>1 mechanic memang bisa terlibat](#order-integration-bersifat-form-centric-bukan-per-fitur) (masing-masing punya `TaskPersonalized` sendiri), tapi mereka mengerjakan unit yang sama bersama-sama — di situlah koordinasi lapangan bekerja, dan di situ "people & process" benar-benar berlaku sebagai mitigasi.
2. **Approval ditargetkan hitungan menit sampai jam**, bukan hari — window-nya pendek.

**Residual yang tetap ada: lintas shift.** eMOL yang disubmit menjelang akhir shift bisa saja masih pending saat Form shift berikutnya dibuka. Di situ batasan 1-Form-per-shift tidak menolong, karena Form-nya memang Form yang berbeda. Peluangnya paling besar di cabang **`IsImmediateExecutable=No`** — kerusakannya belum diperbaiki, jadi masih ada secara fisik untuk ditemukan lagi. Dampaknya paling besar di cabang **`Yes`** — pekerjaannya sudah selesai, jadi Order kedua murni sampah. Dua hal berbeda: `No` lebih **mungkin terjadi**, `Yes` lebih **merugikan kalau terjadi**.

**Diterima untuk sekarang, dan guard-nya sudah diketahui:** pencegahan penuh baru ada saat **fitur reject** tersedia ([Poin 10](#poin-10-rejectrework-flow)) — approver bisa membatalkan Order kedua yang seharusnya tidak ada. Sampai saat itu approver hanya bisa **mengoreksi** tautan reuse, tidak menolak eMOL-nya ([lihat batas makna gate di Poin 5](#poin-5-data-flow-defect-dan-crack)). Perlu dicatat juga bahwa duplicate Order **sudah terjadi hari ini** tanpa fitur ini (`DeleteReason` bernilai `"double order"`, [lihat temuan data Poin 5](#poin-5-data-flow-defect-dan-crack)) — correlation mengecilkan masalahnya secara signifikan, tidak menghilangkannya.
