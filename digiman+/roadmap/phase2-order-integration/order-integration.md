...
**Keputusan:** tambah opsi manual di cabang "tidak ketemu" — user bisa pilih **"saya tahu ada Order SAP untuk ini"**, lalu input **SAP MO Number manual** (mandatory saat opsi ini dipilih). Karena Digiman+ tidak punya data apapun soal Order SAP itu (tidak pernah tersimpan di manapun), **Order Type/Activity Type/Material tetap diisi manual** oleh user (tidak ada yang bisa di-derive, beda dari cabang reuse-dari-suggestion yang materialnya read-only). Defect ini nempel/di-link ke SAP MO Number itu (lewat field manual yang sama dengan [gap-sync sebelumnya](#poin-1-trigger-dan-ui-create-defect-atau-crack)), bukan create Order baru di sisi Digiman+.
...
| **2. Snapshot Order lama di service `maintenance-order`** — hanya kalau tingkat 1 kosong, hanya tersedia untuk **A1** | `MechanicOrderList.CostTypeCode`/`OrderTypeName`, `MechanicOrderList.ActivityTypeCode`/`ActivityTypeName` | `MechanicOrderMaterial` |
...
Alasan tangga ini: perubahan di SAP tidak terlihat dari sisi Digiman+, jadi snapshot di `maintenance-order` (`MechanicOrderList`, `MechanicOrderMaterial`) tidak boleh dipakai selama tabel hasil sync SAP (`MOOpen`, `CheckPartOrder`) punya jawaban.
...
2. **Selain itu** (tidak ada di `MOOpen`) → pakai `SAPMOSyncOrder.SAPStatus`, di-convert BE ke label:
   - `SAPStatus = 1` → **"Success sync to SAP"**
...
2. eMOL ini **lewat Planner approval seperti biasa** (jalur Order-approval existing, tidak ada bypass) — Planner jadi checkpoint preventive atas keputusan reuse mechanic, review data yang sama seperti Order lain (karena sudah full snapshot).
...
**Untuk cabang `No`+reuse — REVERSED (2026-08-16):** sekarang **JUGA create eMOL** "vehicle approval" (publish full Order-event), sama persis dengan cabang `Yes` — bedanya cuma di post-approval action (`Yes` trigger `BacklogExecutionList`, `No` tidak ada aksi lanjutan, [lihat Poin 9](#poin-9-approval-flow)). Update `MONo` Order lama (kalau SAP MO Number opsional diisi) sekarang ikut dalam payload publish penuh ini, bukan publish targeted terpisah lagi.
...
| `PoolingMOItem` tidak ada sama sekali, `MOOpen` ada | Broader Match saja (satu-satunya cara ketemu) | **NULL** (tidak ada row lokal untuk direferensikan) | **Auto-derive dari `MOOpen.MONumber`** — tidak perlu input manual. | Ikut [tangga Sumber Data Order](#user-journey--defect-2026-08-13) — Order Type/Activity Type **selalu** tingkat 1 (`MOOpen` pasti ada, itu yang bikin kandidat ini ketemu). Material tingkat 1 (`CheckPartOrder`) **hanya kalau ada row** — kalau kosong langsung jatuh ke tingkat 3, karena tingkat 2 tidak tersedia (tidak ada row di `maintenance-order`). |
...
**Perlu dicek developer ke data PRD saat implementasi:** `MOOpen.CostTypeCode` dan `MOOpen.MaintenanceCategoryCode` diasumsikan **satu code space** dengan `MechanicOrderList.CostTypeCode` dan `MechanicOrderList.ActivityTypeCode` — masuk akal karena keduanya nilai yang dikirim Digiman+ ke SAP lalu kembali lewat inbound sync ([`MOType` dari `mol.CostTypeCode`](../../architecture/inspection-order/order-emol-sap-sync.md#52-bc-update-jika-ada-create-jika-tidak-ada--insert-ke-poolingmoitem), [`PMActType` dari `MechanicOrderList.ActivityTypeCode`](#poin-12-saperp-integration)). Kalau ternyata beda format, tingkat 1 akan diam-diam menghasilkan Order Type/Activity Type yang salah — tidak ada error, sama sifatnya dengan risiko join `MOOpen.MONumber` ↔ `SAPMOSyncOrder.MONo`.
...
>
> **✅ Dikuatkan (2026-08-15, final setelah beberapa kali revisi):** ini berlaku untuk **SEMUA** sub-kasus, **termasuk Sub-kasus A (reuse)** — sempat disimpulkan Sub-kasus A tidak perlu Planner approval sama sekali (karena tidak ada eMOL baru), tapi ini menghilangkan safety net atas keputusan reuse mechanic di lapangan. **Keputusan final: Sub-kasus A JUGA membentuk eMOL baru** ("vehicle approval", full snapshot data dari Order lama — lihat [Poin 5](#poin-5-data-flow-defect-dan-crack)) yang lewat Planner approval seperti biasa — bedanya cuma di **behavior post-approval** (Sub-kasus A trigger `BacklogExecutionList` ke Order lama, bukan sync SAP sebagai MO baru — supaya tidak duplicate).
...
**2 fakta yang sudah diputuskan sekarang (buat referensi next MVP, meski fitur reject-nya sendiri belum dibangun):**
...
Detail lain (siapa yang bisa reject, UI-nya seperti apa, alasan reject disimpan di mana, dst) **belum dibahas sama sekali** — genuinely di luar scope sampai next MVP.
