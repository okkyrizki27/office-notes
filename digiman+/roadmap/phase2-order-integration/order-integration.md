...
**Guard teknisnya sudah diketahui, tinggal menunggu:** pencegahan penuh baru ada saat **fitur reject** tersedia ([Poin 10](#poin-10-rejectrework-flow)) — approver bisa membatalkan Order kedua yang seharusnya tidak ada. Sampai saat itu approver hanya bisa **mengoreksi** tautan reuse, tidak menolak eMOL-nya ([lihat batas makna gate di Poin 5](#poin-5-data-flow-defect-dan-crack)). Perlu dicatat juga bahwa duplicate Order **sudah terjadi hari ini** tanpa fitur ini (`DeleteReason` bernilai `"double order"`, [lihat temuan data Poin 5](#poin-5-data-flow-defect-dan-crack)) — correlation mengecilkan masalahnya secara signifikan, tidak menghilangkannya.
