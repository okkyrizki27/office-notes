...
**Open item (2026-08-19) — visibilitas eMOL yang menggantung.** Kalau `MONo` tidak pernah datang karena sync tersendat, approver **tidak punya cara tahu harus menunggu berapa lama** — dari layarnya, sync yang telat sebentar dan sync yang genuinely stuck terlihat sama. **Tidak buntu** (escape valve di atas selalu tersedia), tapi tanpa visibilitas approver akan cenderung memakai escape valve — dan escape valve itulah yang menghasilkan catatan tidak akurat plus Order lama yang muncul lagi sebagai outstanding backlog.

**Dua kasus tersamar jadi satu di baris 3, dan bobotnya beda:** **(i) sementara** — Order baru saja approved, sync masih jalan, menunggu memang menolong (jarang, karena berarti me-reuse Order yang baru disetujui beberapa menit lalu); **(ii) permanen** — sync-nya gagal dan `MONo` tidak akan pernah datang. Untuk (ii) menunggu **tidak pernah** cukup, dan Order itu **tetap jadi kandidat selamanya** karena candidate scope di-drive dari union `PoolingMOItem` ∪ `MOOpen` — row `PoolingMOItem`-nya tetap ada. Jadi escape valve bukan dipakai sekali, tapi berulang untuk Order yang sama. Catatan diagnostik: `PoolingMOItem` ada + `MOOpen` tidak ada dalam waktu lama berarti **kedua** jalur sync sama-sama tidak membawa nomornya — indikasi kuat Order-nya tidak pernah terbentuk di SAP, sehingga membuat Order baru justru lebih tepat daripada me-reuse-nya.

**Keputusan (2026-08-19) — dibiarkan apa adanya, monitoring di-defer ke next MVP.** Sifatnya sama dengan sync tersendat lainnya, jadi masuk **monitoring umum**, bukan desain khusus fitur ini. Sejalan dengan itu: notifikasi in-app/email memang [out of scope Phase 2](#poin-8-cancel-atau-delete-finding), dan jalur perbaikan manual (input `MONo` oleh user) sudah dihapus di [Poin 6](#poin-6-duplicate-atau-correlation-handling) — jadi di dalam fitur ini tidak ada deteksi maupun perbaikan; keduanya bersandar ke monitoring umum. Residual sejenis sudah dicatat untuk scheduler B2 ([Poin 1](#poin-1-trigger-dan-ui-create-defect-atau-crack)).

**Alternatif yang dipertimbangkan dan sengaja tidak diambil sekarang:** meng-**exclude A1 yang `MONo`-nya NULL terlalu lama dari candidate scope** — pola yang sama persis dengan exclude status terminal ([lihat candidate scope Poin 6](#poin-6-duplicate-atau-correlation-handling)), dan cukup satu klausa tambahan di query tanpa UI atau notifikasi baru. Tidak diambil di Phase 2 karena butuh ambang waktu yang belum punya dasar operasional. Dicatat di sini supaya tidak perlu digali ulang kalau kasus (ii) ternyata sering muncul setelah rilis.
